...
- `/ordering/recommendations` 컨텍스트 카드의 `최근 트렌드` 슬롯을 **기준일자 진행 중 캠페인** 카드로 교체했습니다.
  - 카드 헤더에 `<input type="date">` 기반 기준일자 피커를 두고, 선택한 날짜에 진행 중(`start_dt ≤ ref_date ≤ fnsh_dt`)인 캠페인을 최대 3건 불릿 리스트로 노출합니다.
  - 신규 query 훅: `src/features/ordering/queries/useGetOrderingActiveCampaignsQuery.ts` (`GET /api/ordering/active-campaigns`).
  - 카드 컴포넌트: `src/features/ordering/components/OrderingContextCards.tsx` (props 변경: `trend` 제거 → `campaigns / referenceDate / onChangeReferenceDate / isCampaignsLoading`).
  - 화면 연결: `src/features/ordering/screens/OrderingRecommendationsScreen.tsx`에서 기준일자 상태(`useState`, 기본값 오늘)를 카드와 쿼리에 동시 전달.
  - 빈 결과/로딩/오류 상태 처리: 빈 결과는 "진행 중인 캠페인 없음", 로딩은 3줄 스켈레톤.

## 최근 업데이트 (2026-04-25)

- backend 전체 QA/치명 회귀 보강 세션 연동 사항을 반영했습니다.
  - 이번 세션의 프론트 코드 변경은 없습니다.
  - backend `/api/analytics/metrics`의 fallback 안정화 및 라벨 정합(투고 매출액/홀 방문 고객) 변경을 기존 프론트 렌더 구조에서 그대로 수용합니다.

- backend `inventory-status` 리팩토링 연동 사항을 동기화했습니다.
  - `business_date` 기준 캐시 분리와 CTE 통합은 backend 내부 변경이며, 이번 세션의 프론트 코드 변경은 없습니다.

- `/sales/metrics`에 **활성 캠페인 효과 카드**(`SalesV2CampaignActivitySection`)를 추가했습니다. 백엔드 `GET /api/sales/campaign-effect` 응답의 `title/summary/metrics/actions`를 그대로 카드로 렌더링하며, 기준 기간에 활성 캠페인이 있을 때(활성 N건 / Top / 매출/할인 배수 / 평균 lift)와 없을 때(최근 90일 운영 캠페인 표시) 모두 자연스럽게 노출됩니다.
  - 구현: `src/features/sales/components/SalesV2CampaignActivitySection.tsx`, `src/features/sales/screens/SalesScreenV2.tsx`, `src/features/sales/hooks/useSalesScreenV2.ts`
  - 타입: `src/features/sales/types/sales.ts`의 `SalesCampaignEffectResponse`에 `title/summary/metrics/actions`(옵셔널) 필드를 추가해 백엔드 schema와 정합화했습니다.

- `/analytics` 매출 현황 화면 하단 KPI 그리드를 운영 KPI 7종으로 재구성했습니다 (앱 주문 비중·할인 결제 비중·선택 기간 총 매출·평균 객단가 제거, 투고 건수·배달 매출액 분리, 추정치 "홀 방문 고객"을 실측 `매장 결제 건수`(배달/해피오더 제외)로 변경, 런치/스윙/디너 시간대 평균 단가 추가). `/sales/metrics`는 손익 분석/기회 영역 화면으로 역할을 분리합니다.
  - 구현: `src/features/analytics/components/AnalyticsMetricsGrid.tsx`(items 자동 렌더링), `GET /api/analytics/metrics` 응답 변경
- 헤더 매장명이 `POC 010 매장`으로 잘못 표시되던 버그를 수정했습니다.
  - `Store.tsx` `useEffect`가 `user.storeId`만 검증하던 로직을 `store_name` 동기화까지 수행하도록 보강해 `/api/stores` 응답을 강제 반영합니다.
  - `.env`/`.env.example`/`docker-compose.yml`의 `VITE_DEFAULT_STORE_NAME` 기본값을 실제 매장명(`강서구01점`/`마포구01점`)으로 정렬했습니다.
  - 구현: `src/commons/components/layout/header/Store.tsx`
- backend/ai 치명 버그 수정 세션을 연동 기준으로 반영했습니다.
- FloatingAiChat 코드 정리 및 중복 제거 리팩토링을 적용했습니다.
  - 미사용 `buildFloatingAiReply` 스텁(`src/commons/utils/build-floating-ai-reply.ts`)을 삭제했습니다.
  - localStorage 세션 관리·route 해석 로직을 `src/commons/utils/floating-ai-chat-session.ts`로 분리했습니다.
  - `extractCandidateText`, `dedupeStrings` 유틸을 `src/commons/utils/chat-text-utils.ts`로 분리했습니다.
  - `ChatHistoryEntry` 타입은 `src/commons/types/floating-ai-chat.ts`에 단일 정의하고 `src/features/sales/types/sales.ts`에서 re-export 합니다.
  - `FloatingAiChatReference.processingRoute` 중복 필드를 제거하고 `FloatingAiChatMessage.processingRoute`만 유지합니다.
  - 구현: `src/commons/components/chat/FloatingAiChat.tsx`, `src/commons/types/floating-ai-chat.ts`
- FloatingAiChat 쿼리 페이로드를 실행 컨텍스트 기반으로 확장했습니다.
  - `POST /api/sales/query` 본문에 `business_date`, `business_time`, `page_context`, `card_context_key`, `store_name`, `user_role`, `conversation_history`(직전 6턴) 필드를 추가했습니다.
  - `postSalesQuery` 시그니처를 `(prompt, storeId, domain, context)` 객체 옵션 방식으로 변경했습니다.
  - 구현: `src/features/sales/api/sales.ts`, `src/features/sales/types/sales.ts`, `src/commons/components/chat/FloatingAiChat.tsx`
- FloatingAiChat을 실제 AI API 연동 + 세션 유지 구조로 전환했습니다.
  - 스텁 응답 대신 `postSalesQuery`를 호출하고 응답의 `agent_trace`, `data_lineage`, `semantic_logic`을 레퍼런스 패널에 노출합니다.
  - pathname 기반 sessionKey로 localStorage에 도메인별 대화 이력을 유지합니다.
  - 골든쿼리 매칭 ID/미매칭 배지와 추천 후속 질문(`overlap_candidates` + golden prompts) 칩을 표시합니다.
  - 구현: `src/commons/components/chat/FloatingAiChatPanel.tsx`, `src/commons/types/floating-ai-chat.ts`
- 매출 기회 분석 카드 4종에 InfoPopover를 추가했습니다.
  - 마케팅 ROI / 채널 결제 / 프로모션 효율 / 매장 벤치마킹 카드 헤더에 정의·가정·산식을 노출합니다.
  - `FIELD_CAPTIONS`에 `sales:opportunity_*` 키 4종을 신규 추가했습니다.
  - 구현: `src/features/sales/components/SalesV2OpportunitySection.tsx`, `src/commons/constants/field-captions.ts`
- 대시보드 SummaryCard 추천 질문 클릭 시 도메인 컨텍스트를 라우팅 state로 전달합니다.
  - `state: { source: "dashboard-card-chat", domain, intent, prompt }`를 카드별 경로로 넘겨 FloatingAiChat이 즉시 질의를 실행합니다.
  - `SummaryCardProps`에 `domain: DashboardDomain` 필드를 추가했습니다.
  - 구현: `src/features/dashboard/components/SummaryCard.tsx`, `src/features/dashboard/components/SummaryCardsSection.tsx`, `src/features/dashboard/types/summary-card.ts`
- 생산 현황 테이블 머지 충돌로 손실된 InfoPopover 컬럼을 복원했습니다.
  - 주문 마감 시간 / 4주 평균 1차·2차 생산량 컬럼과 `production:forecast_1h`, `production:avg_first_prod_4w`, `production:avg_second_prod_4w`, `production:chance_loss` InfoPopover를 복구했습니다.
  - 구현: `src/features/production/components/ProductionTableSection.tsx`
- 상권 분석 구/동 필터 옵션을 동적 API 조회 기반으로 확장했습니다.
  - `MarketScopeOptionsResponse` 타입과 `getAnalyticsMarketScopeOptions` API, `analyticsQueryKeys.marketScopeOptions` 키를 추가했습니다.
  - 구현: `src/features/analytics/api/analytics.ts`, `src/features/analytics/queries/analyticsQueryKeys.ts`, `src/features/analytics/types/analytics.ts`
- 백엔드/AI 반복 예외 처리 패턴 제거 리팩토링 세션을 연동 기준으로 반영했습니다.
  - 사용자 요청에 따라 이번 세션의 프론트 소스 코드는 변경하지 않았습니다.
- 매출 차트 섹션 렌더링 리팩토링을 반영했습니다.
  - `SalesV2ChartsSection` 내부에서 생성하던 `CustomTreemapContent`를 파일 스코프로 분리해 렌더 시 컴포넌트 재생성 이슈를 제거했습니다.
  - 구현: `src/features/sales/components/SalesV2ChartsSection.tsx`
- 상권 필터를 5개 구 고정에서 동적 범위로 확장했습니다.
  - `GET /api/analytics/market-scope-options` 응답으로 `구/동` 목록을 주입합니다.
  - 백엔드 응답이 없을 때도 서울 25개 구 기본 목록을 fallback으로 노출합니다.
  - 구현: `src/features/analytics/components/AnalysisScopeFilterBar.tsx`, `src/features/analytics/screens/MarketScreen.tsx`, `src/features/analytics/queries/useGetAnalyticsMarketScopeOptionsQuery.ts`
- `MarketCustomerSection` 중복 key 경고를 수정했습니다.
  - `customer_segments`, `telecom_discounts` 반복 렌더링 key를 중복 데이터에도 충돌하지 않도록 보강했습니다.
  - 구현: `src/features/analytics/components/MarketCustomerSection.tsx`
- 사이드바 active 충돌을 수정했습니다.
  - `/analytics/market` 진입 시 `/analytics`까지 동시에 active 처리되던 문제를 수정했습니다.
  - 메뉴 경로 중 현재 URL과 가장 구체적으로 일치하는 1개 항목만 active 됩니다.
  - 구현: `src/commons/components/layout/AppSidebar.tsx`
- 상권 분석 운영 액션 가이드 fallback 렌더링을 제거했습니다.
  - AI 인사이트(`market-intelligence/insights`) 미수신/오류 시 프론트 임의 액션 문구를 생성하지 않습니다.
  - 인사이트 실패 시 오류 상태 메시지를 표시하고 mock성 가이드를 노출하지 않습니다.
  - 구현: `src/features/analytics/components/MarketActionGuideSection.tsx`, `src/features/analytics/screens/MarketScreen.tsx`
- backend `sales` API 안정화 패치(요약 no-data 200, AI 미연결 시 기본 인사이트 응답)가 반영되었습니다.
  - 프론트 `/sales/metrics`에서 `summary/insights/campaign-effect` 연속 실패가 발생하던 케이스를 완화합니다.

## 최근 업데이트 (2026-04-24)

- 생산 진단 이월재고(FIFO) 집계 기준을 당일 기준으로 동기화했습니다.
  - `/production/inventory-diagnosis`의 `GET /api/production/fifo-lots` 응답은 기준일 `lot_date` 1일치 집계로 표시됩니다.
  - FIFO 툴팁 문구를 기준일 1일치 집계 의미로 정리했습니다.
...
- 기본 환경값은 `VITE_DEFAULT_STORE_ID=POC_010`, `VITE_DEFAULT_REFERENCE_DATETIME=2026-03-05T09:00`입니다.
...
## Session Update (2026-04-25, settings summary cards white background)

- `/settings` 주요 패널의 상단 요약 카드 배경을 흰색으로 통일했습니다.
  - connectors 요약 4카드
  - golden queries 요약 4카드
  - audit logs 요약 4카드
  - quality archive Agent 점수 3카드
- 변경 파일: `src/features/admin/orchestration/components/ConnectorsPanelV3.tsx`, `GoldenQueriesPanel.tsx`, `AuditPanelV3.tsx`, `QualityPanelV3.tsx`
- 검증: `npm run build` 통과

## Session Update (2026-04-25, prompts textarea width adjustment)

- `/settings/prompts`의 프롬프트 카드 영역을 2열(`g2`)에서 1열 그리드로 조정해 textarea 가로 폭을 확장했습니다.
- 변경 파일: `src/features/admin/orchestration/components/PromptsPanel.tsx`
- 참고: 현재 build는 별개 파일의 미사용 변수 TS6133 에러(`FloatingAiChatPanel.tsx`, `ProductionTableSection.tsx`)로 실패합니다.

## Session Update (2026-04-25, prompts card equal height)

- `/settings/prompts`의 4개 프롬프트 카드 높이를 동일하게 고정해 높낮이 차이를 제거했습니다.
- 각 textarea는 카드 내부에서 `flex-1`로 남은 높이를 채우도록 조정했습니다.
- 변경 파일: `src/features/admin/orchestration/components/PromptsPanel.tsx`
- 참고: build는 기존 별개 TS6133 미사용 변수 이슈로 실패합니다.

## Session Update (2026-04-25, prompts card height 80 + equal input height)

- `/settings/prompts` 4개 카드를 `h-80`으로 통일했습니다.
- 카드 내부 입력 textarea도 동일 높이(`h-40`)로 고정하고 `resize-none`으로 통일했습니다.
- 변경 파일: `src/features/admin/orchestration/components/PromptsPanel.tsx`
- 참고: build는 기존 별개 TS6133 미사용 변수 이슈로 실패합니다.

## Session Update (2026-04-25, floating chat golden-query integration)

- 플로팅 챗을 정적 응답에서 API 기반 질의 실행으로 전환했습니다.
...
- 변경 파일:
  - `src/commons/components/chat/FloatingAiChat.tsx`
  - `src/commons/components/chat/FloatingAiChatPanel.tsx`
  - `src/commons/types/floating-ai-chat.ts`
  - `src/features/sales/types/sales.ts`
- 추가로 빌드 회귀 이슈(TS6133)였던 미사용 prop을 정리했습니다.
  - `src/features/production/components/ProductionTableSection.tsx`
  - `src/features/production/screens/ProductionStatusScreen.tsx`
- 검증: `npm run build` 통과

## Session Update (2026-04-25, floating chat source badge + reference popup)

- 플로팅 챗 답변 버블에 `출처` 배지를 추가하고 클릭 시 팝업에서 출처/근거/참고 쿼리 정보를 노출합니다.
- AI 응답 메타 중 `sources`, `semantic_logic`, `agent_trace(sql/relevant_tables/queried_period/match)`를 프론트 메시지 reference로 매핑합니다.
- 변경 파일: `src/commons/components/chat/FloatingAiChat.tsx`, `src/commons/components/chat/FloatingAiChatPanel.tsx`, `src/commons/types/floating-ai-chat.ts`, `src/features/sales/types/sales.ts`
- 검증: `npm run build` 통과

## Session Update (2026-04-25, golden query miss badge)

- 플로팅 챗 답변에서 `processing_route`가 `golden_query_miss*`이고 `matchedQueryId`가 없을 때 `골든쿼리 미매칭` 배지를 표시합니다.
- 변경 파일: `src/commons/components/chat/FloatingAiChatPanel.tsx`
- 검증: `npm run build` 통과

## Session Update (2026-04-25, dashboard recommended question handoff to floating chat)

- `/dashboard` 요약 카드의 추천 질문 클릭 시 단순 페이지 이동이 아니라 `dashboard-card-chat` route state(`intent=ask`, `prompt`)를 함께 전달합니다.
- 도착 페이지에서 플로팅 챗이 자동으로 열리고 전달된 질문을 즉시 `/api/sales/query`로 실행하도록 연결했습니다.
- 변경 파일: `src/features/dashboard/components/SummaryCard.tsx`, `src/features/dashboard/components/SummaryCardsSection.tsx`, `src/features/dashboard/types/summary-card.ts`, `src/commons/components/chat/FloatingAiChat.tsx`
- 검증: `npm run build` 통과

## Session Update (2026-04-25, market page sales-trend card removal)

- `/analytics/market` 화면에서 `매출 추이 분석`(`SalesTrendChart`) 카드를 제거했습니다.
- 다른 섹션 레이아웃과 데이터 조회 흐름은 유지했습니다.
- 변경 파일: `src/features/analytics/screens/MarketScreen.tsx`
- 검증: `npm run build` 통과

## Session Update (2026-04-25, sales metrics info-popover coverage)

- `/sales/metrics` 화면의 주요 카드 헤더에 `i`(InfoPopover) 설명을 일괄 적용했습니다.
  - 주간 매출/순매출 추이, 주간 매출 구성, 오늘 수익 구성, 상위 상품 매출, 핵심 지표 현황, 상품별 매출 비중
  - 인사이트 카드
  - 기회 카드(마케팅 ROI, 채널/결제 최적화, 프로모션 효율, 벤치마킹)
- `FIELD_CAPTIONS`에 매출 지표/인사이트/기회 카드용 설명 키를 추가했습니다.
- 변경 파일:
  - `src/features/sales/components/SalesV2ChartsSection.tsx`
  - `src/features/sales/components/SalesV2InsightsSection.tsx`
  - `src/features/sales/components/SalesV2OpportunitySection.tsx`
  - `src/commons/constants/field-captions.ts`
- 검증: `npm run build` 통과

## Session Update (2026-04-25, floating chat suggested questions pinned to golden prompts)

- 플로팅 챗 후보 질문 구성을 골든쿼리 프롬프트(`GET /api/sales/prompts`) 중심으로 고정했습니다.
- `follow_up_questions` 문구는 후보 질문 우선순위에서 제외해 재귀형 문장(예: "...를 최근 7일 기준으로 다시 보여줘를...")이 노출되지 않도록 정리했습니다.
- 후보 질문 우선순위: `overlap_candidates` -> `golden prompts` (미수신 시 route 기본 quick action fallback)
- 변경 파일: `src/commons/components/chat/FloatingAiChat.tsx`
- 검증: `npm run build` 통과

## Session Update (2026-04-25, ordering history chart date-axis alignment)

- `/ordering/history` 화면 차트의 날짜 축을 필터 기간(`date_from ~ date_to`) 전체로 고정했습니다.
- 납품일 데이터가 없는 날짜도 `0` 값으로 채워 시작일/종료일과 차트 일자가 어긋나 보이던 문제를 수정했습니다.
- 변경 파일:
  - `src/features/ordering/components/OrderingHistoryChartsSection.tsx`
  - `src/features/ordering/screens/OrderingHistoryScreen.tsx`

## Session Update (2026-04-25, SPECIAL ordering basis removal 영향도)

- AI 주문 추천 계약에서 `SPECIAL(특별 기간)` 옵션 타입이 제거되었습니다.
- 프론트 코드 변경은 없으며 기존 3개 추천안 렌더링 흐름을 그대로 사용합니다.

## Session Update (2026-04-25, reference datetime default 09:00)

- 데모 세션 기준일시 기본값을 `2026-03-05T09:00`으로 조정했습니다.
- 헤더 기본값 안내 문구를 `기본값: 3월 5일 09:00 (오전)`으로 변경했습니다.
- Docker 기본 환경변수(`VITE_DEFAULT_REFERENCE_DATETIME`)도 `2026-03-05T09:00`으로 동기화했습니다.

## Session Update (2026-04-25, weekly revenue x-axis date+weekday tilt)

- `/sales/metrics`의 `주간 매출 / 순매출 추이`, `주간 매출 구성` X축 라벨을 `YYYY-MM-DD(요일)` 형식으로 확장했습니다.
- X축 라벨이 겹치지 않도록 라벨 각도를 기울여(약 -35도) 엑셀 스타일로 가독성을 개선했습니다.
- 변경 파일:
  - `src/features/sales/components/SalesV2ChartsSection.tsx`
- 검증: `npx eslint src/features/sales/components/SalesV2ChartsSection.tsx` 통과

## Session Update (2026-04-25, treemap chart height reduction)

- `/sales/metrics`의 `상품별 매출 비중` Treemap 높이를 `180 -> 140`으로 축소해 세로 길이를 줄였습니다.
- 변경 파일: `src/features/sales/components/SalesV2ChartsSection.tsx`
- 검증: `npx eslint src/features/sales/components/SalesV2ChartsSection.tsx` 통과

## Session Update (2026-04-25, analytics KPI curation 영향도)

- `/analytics` 하단 KPI 카드 데이터 소스(`/api/analytics/metrics`) 라벨/지표 구성이 서버 기준으로 변경되었습니다.
- 프론트 컴포넌트 구조 변경 없이 기존 `AnalyticsMetricsGrid` 렌더링을 그대로 사용합니다.
- `/sales/metrics` 화면 구성은 변경되지 않습니다.

## Session Update (2026-04-25, takeout/delivery share 표시 영향도)

- `/analytics` 하단 KPI의 `투고 매출액`, `배달 매출액` 카드 detail에 전체 매출 대비 비중 문구가 backend 응답 기준으로 표시됩니다.
- 프론트 컴포넌트 구조 변경은 없습니다.

## Session Update (2026-04-26, backend QA stabilization sync)

- 이번 세션은 backend 치명 회귀 복구 중심으로 진행했으며 프론트 코드 변경은 없습니다.
- 기존 API 연동 계약(`/api/production`, `/api/ordering`, `/api/sales`)과 화면 구조는 유지됩니다.

## Session Update (2026-04-26, frontend lint green)

- lint 오류 11건을 정리했습니다.
- `setState-in-effect` 패턴을 파생 상태/핸들러 기반으로 전환했고, unused 변수와 정규식/Fast Refresh 규칙 위반을 수정했습니다.
- 검증: `npm run lint` 통과
