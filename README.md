...
- `/settings/connectors`에 현재 시스템 DB 기준 객체(raw/core/운영) 30개를 기존 커넥터 카드와 동일한 스타일로 반복 표시하도록 확장했습니다.
  - 기존 Settings v3 톤앤매너를 유지하고, 테이블/뷰 단위 카드(30개)가 같은 레이아웃으로 렌더링됩니다.

- 플로팅챗 골든쿼리 고도화(backend/ai) 연동 사항을 문서 동기화했습니다.
  - 채팅 응답 계약에 `follow_up_questions` `overlap_candidates`가 확장되어 유사 골든쿼리 유도 UX를 지원합니다.
  - 이번 세션에서는 프론트 소스 코드는 추가 변경하지 않았고 기존 계약 호환을 유지합니다.

- 주문 추천 고도화(백엔드/AI) 반영: 프론트 UI 디자인 코드는 변경하지 않았습니다.
  - `/ordering/recommendations`는 기존 화면 구조 그대로, 백엔드가 제공하는 강화된 `reasoning_metrics`/`trend_summary`/`item.note`를 표시합니다.
  - 추천 수량 산식은 서버에서 `최근 판매추세 + 현재 재고 + 유통기한 리스크`를 반영하도록 변경되었습니다.
- 주문 추천 리팩토링(backend/ai) 후속 반영: 프론트 UI 디자인 코드는 변경하지 않았습니다.
  - 주문 추천의 계산/병합 정책 정리는 서버 레이어 변경이며, 프론트는 기존 계약 렌더링만 유지합니다.
...
## Session Update (2026-04-24, backend+ai golden query routing)

- 이번 라운드는 backend+ai의 골든쿼리 우선 라우팅 구현 세션입니다.
- 프론트 코드 변경은 없고, 기존 `/api/sales/query` 계약을 그대로 사용합니다.
- 백엔드 응답 `agent_trace`에 골든쿼리 추적 필드(`matched_query_id`, `match_score`)가 선택적으로 포함될 수 있습니다.

## Session Update (2026-04-24, backend+ai only scope)

- 이번 라운드는 backend+ai 변경만 수행했습니다.
- 프론트 코드 변경은 없습니다.

## Session Update (2026-04-24, backend+ai pattern matching scope)

- 이번 라운드는 backend+ai 범위 작업이며, 프론트 코드 변경은 없습니다.
- 골든쿼리 매칭 강화는 서버측에서 처리되어 기존 프론트 호출 계약은 유지됩니다.
