...
export const FIELD_CAPTIONS: Record<string, FieldCaption> = {
  ...
  "production:recommended_qty": {
    assumption:
      "오늘 마감까지 예상되는 찬스 로스를 최소화할 만큼의 부족 수량을 우선 채우고, 평소 1차·2차 생산 평균 수량과 크게 어긋나지 않으면 그 생산 단위에 맞춰 권고 수량을 올림합니다.",
    formula:
      "기본 필요 수량 = ceil(오늘 남은 시간대 예상 부족 수량)\n" +
      "추천 생산 수량 = 기본 필요 수량 이상이면서, 평소 1차·2차 평균 생산 수량 또는 그 합계에 근접한 가장 작은 수량",
    description: "오늘 남은 시간의 잠재 매출 손실을 줄이면서도 평소 생산 패턴을 참고한 권고 생산 수량입니다.",
  },
  // 생산 현황 - 현재 판매 속도
  ...
  // 폐기 손실 - 품목명
  "production:waste_item_name": {
    assumption:
      "품목명은 폐기 손실 원본 데이터의 품목명을 기준으로 표시합니다.\n" +
      "• [JBOD] 제품은 비교용 정규화 단계에서 앞의 [JBOD] 접두어를 제거합니다.\n" +
      "• 정규화 시 공백과 문자폭 차이도 함께 정리해 판매·단가 데이터와 매칭할 때 사용합니다.\n" +
      "• 화면에는 정규화명이 아니라 원본 품목명을 그대로 보여줍니다.",
    description:
      "[JBOD] 표기가 있는 품목도 내부 비교용 키에서는 접두어를 제거해 연관 판매·단가 데이터와 매칭합니다.",
  },
  // 폐기 손실 - 폐기한 갯수 (테이블 헤더)
  ...
  "production:forecast_1h": {
    assumption:
      "raw/스냅샷 경로에서는 현재 재고에서 1시간 판매 예상 수량을 차감한 잔여 재고를 사용합니다. 1시간 판매 예상 수량은 최근 4주 같은 요일 시간대 판매 패턴을 기본으로 하고, 같은 군집의 타 지점 동일 시간대 평균 판매량을 보조 신호로 반영합니다. 마트 재고 테이블 경로는 1시간 판매 예측값이 없어 현재 재고를 그대로 표시합니다.",
    formula:
  ...
  "production:chance_loss": {
    assumption:
      "최근 4주 같은 요일 시간대 판매 패턴에 오늘 판매 속도를 반영해, 현재 시각부터 마감까지 부족해질 판매 수량을 계산합니다. 시간대 패턴에는 같은 군집의 타 지점 동일 시간대 평균 판매량도 보조 신호로 반영합니다.",
    formula:
      "시간대 예상 판매 = 최근 4주 같은 요일 시간대 평균 판매량 × 오늘 판매 속도\n" +
      "찬스 로스 수량 = 현재 재고가 소진된 뒤 마감까지 부족해지는 예상 판매 수량의 합\n" +
      "찬스 로스 절감액 = min(추천 생산 수량, 찬스 로스 수량) × 평균 판매 단가",
    description: "오늘 마감까지 적시 생산으로 방지할 수 있는 잠재 매출 손실 추정치입니다.",
  },
