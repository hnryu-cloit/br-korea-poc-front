...
export function OrderingHistoryInsightsSection({ data, isLoading }: Props) {
  ...
  const hasTopChangedItems = (data?.top_changed_items ?? []).length > 0;

  return (
    <section className="rounded-[6px] border border-[#DADADA] bg-white p-5 flex flex-col gap-2">
      <h2 className="text-sm font-bold text-black">이상징후 분석</h2>
      {isLoading ? null : !hasAnomalies && !hasTopChangedItems ? (
        <div className="text-center text-sm text-slate-400">이상 징후 데이터가 없습니다.</div>
      ) : (
        <div className="grid gap-2 lg:grid-cols-2">
          <article className="rounded-[8px] border border-[#DADADA] bg-white p-6">
            <span className="inline-flex items-center gap-2">
              ...
              <InfoPopover caption={FIELD_CAPTIONS["ordering:anomalies"]} side="top" align="left" />
            </span>
            ...
            {hasAnomalies ? (
              ...
              <p className="py-8 text-center text-sm text-slate-400">이상징후가 없습니다.</p>
            )}
          </article>
          <article className="rounded-[8px] border border-[#DADADA] bg-white p-6">
            ...
            {hasTopChangedItems ? (
              ...
              <p className="py-8 text-center text-sm text-slate-400">주요 변동 품목이 없습니다.</p>
            )}
          </article>
        </div>
      )}
