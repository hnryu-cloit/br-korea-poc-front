...
import { getLogPromptText, getLogQueryLabel } from "@/features/analytics/utils/analytics-screen";
  ...
  <section className="overflow-hidden rounded-[28px] border border-border bg-white shadow-[0_12px_30px_rgba(16,32,51,0.06)]">
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-6 py-5">
      <div>
        ...
        <p className="mt-0.5 text-xs text-slate-400">
          매출 도메인 · SQL/API 우선 처리율 {sqlPct}% · 차단{" "}
          {formatCountWithUnit(blockedCount, "건")}
        </p>
      </div>
          ...
          <button
            ...
            className={`rounded-full px-3 py-1.5 text-xs font-semibold transition-colors ${
              activeCategory === category
                ...
                : "border border-[#dce4f3] bg-[#f7faff] text-slate-600 hover:bg-[#eef4ff]"
            }`}
          >
            {category}
          </button>
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-4 text-center">
                    <span
                      className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ${ROUTE_STYLE[log.route] ?? "bg-slate-100 text-slate-600"}`}
                    >
    ...
    <div className="border-t border-border/40 px-6 py-4">
      <div className="flex items-center justify-between text-xs text-slate-400">
        <span>
          매출 도메인 총 {formatCountWithUnit(allLogsCount, "건")} 처리 · SQL/API 우선 처리율{" "}
          {sqlPct}%
        </span>
