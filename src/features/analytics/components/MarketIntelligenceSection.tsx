      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h3 className="mt-1 text-lg font-bold text-slate-900">
            반경 {data?.radius_km ?? 3}km 경쟁·유동인구 종합 분석
          </h3>
        </div>
        <p className="text-xs text-slate-500">
          지점별 상권/경쟁 데이터 + 오픈업 결제건 + 서울시 공공데이터 기준
        </p>
      </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
                ...
                {data.category_sales_pie.map((slice) => (
                  <div key={slice.category}>
                    <div className="mb-1 flex items-center justify-between text-xs text-slate-600">
                      ...
                      <span>
                        {slice.share_ratio.toFixed(1)}% · {formatWonCompact(slice.sales_amount)}
                      </span>
                    </div>
                    <div className="h-2 rounded-full bg-slate-100">
                      <div
                        className="h-2 rounded-full bg-[linear-gradient(90deg,#285ed3_0%,#5aa3ff_100%)]"
                        style={{ width: `${slice.share_ratio}%` }}
                      />
                    </div>
                  </div>
                ))}
            ...
            <article className="rounded-2xl border border-[#e5ecfa] bg-[#fbfdff] p-4">
                  ...
                  <div key={point.month} className="space-y-1 text-center">
                    <div className="mx-auto flex h-20 w-6 items-end rounded-md bg-slate-100">
                      <div
                        ...
                        style={{
                          height: `${Math.max(12, (point.floating_population / maxFloatingPopulation) * 100)}%`,
                        }}
                      />
                    </div>
              ...
              <p className="mt-3 text-xs leading-relaxed text-slate-600">
                {data.floating_population_analysis}
              </p>
            </article>
          </div>
                  ...
                  <PieChart>
                    ...
                    >
                      {data.household_composition_pie.map((item, index) => (
                        <Cell
                          key={item.household_type}
                          fill={householdPieColors[index % householdPieColors.length]}
                        />
                      ))}
                    </Pie>
                    <Tooltip
                      formatter={(value, _name, payload) => {
                        ...
                        return [
                          `${Number(value).toFixed(1)}% (${Number(item?.household_count ?? 0).toLocaleString()}가구)`,
                          "비중",
                        ];
                      }}
                    />
            ...
            <article className="rounded-2xl border border-[#e5ecfa] bg-[#fbfdff] p-4">
              <div className="flex items-start justify-between gap-2">
                ...
                <p className="text-[11px] text-slate-500">
                  상위 {data.estimated_residence_regions.length}개 권역
                </p>
              </div>
              <div className="mt-3 space-y-2">
                ...
                <table className="min-w-full text-center text-xs">
                  <thead>
                    <tr className="text-slate-500">
                      ...
                      {["월", "화", "수", "목", "금", "토", "일"].map((dow) => (
                        <th key={dow} className="px-1 py-1">
                          {dow}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {["08-11", "11-14", "14-18", "18-22"].map((hourBand) => (
                      <tr key={hourBand}>
                        <td className="px-2 py-1 text-left font-medium text-slate-600">
                          {hourBand}
                        </td>
                        {["월", "화", "수", "목", "금", "토", "일"].map((dow) => {
                          ...
                          return (
                            <td key={`${hourBand}-${dow}`} className="px-1 py-1">
                              <div
                                className={`rounded-md px-1.5 py-1 font-semibold ${heatmapCellColor(index)}`}
                              >
                                {index}
                              </div>
                            </td>
            ...
            <div className="space-y-3">
              {data.competitors.map((competitor) => {
                const maxSales = Math.max(
                  ...competitor.sales_trend.map((point) => point.sales_amount),
                  1,
                );
                const ageTop = competitor.payment_demographics[0];
                const ageTotal = ageTop
                  ? ageTop.male_payment_count + ageTop.female_payment_count
                  : 0;
                return (
                  <div
                    key={`${competitor.rank}-${competitor.store_name}`}
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-slate-500">
                          {competitor.brand_name} · {competitor.distance_km.toFixed(2)}km
                        </p>
                      </div>
                      <span
                        className={`inline-flex rounded-full px-2 py-1 text-[11px] font-semibold ${trendColor(competitor.trend_direction)}`}
                      >
                    ...
                    <div className="mt-3 grid gap-3 lg:grid-cols-[1.8fr_1fr]">
                          ...
                          <div key={point.month} className="space-y-1 text-center">
                            <div className="mx-auto flex h-12 w-4 items-end rounded bg-slate-100">
                              <div
                                ...
                                style={{
                                  height: `${Math.max(10, (point.sales_amount / maxSales) * 100)}%`,
                                }}
                              />
                            </div>
                            <p className="text-[10px] text-slate-500">{point.month.slice(5)}</p>
                      ...
                      <div className="rounded-lg bg-[#f6f9ff] px-3 py-2">
                        ...
                        <p className="mt-1 text-xs text-slate-600">
                          {ageTop?.age_group ?? "-"} · 총 {ageTotal.toLocaleString()}건
                        </p>
                        <p className="text-xs text-slate-500">
                          남 {ageTop?.male_payment_count.toLocaleString() ?? 0}건 / 여{" "}
                          {ageTop?.female_payment_count.toLocaleString() ?? 0}건
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <article className="rounded-2xl border border-[#e5ecfa] bg-[#fbfdff] p-4">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {data.data_sources.map((source) => (
                <span
                  key={source}
                  className="rounded-full bg-[#edf3ff] px-3 py-1 text-xs text-[#214faf]"
                >
                  {source}
                </span>
              ))}
