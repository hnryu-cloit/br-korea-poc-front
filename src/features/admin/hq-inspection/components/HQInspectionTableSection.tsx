            ...
            {isLoading ? (
              ...
              inspections.map((insp) => {
                ...
                return (
                  <tr
                    key={insp.store}
                    className={`border-b border-border/30 last:border-0 ${insp.status === "noncompliant" ? "bg-red-50/20" : "hover:bg-[#f8fbff]"}`}
                  >
                    ...
                    <td className="px-4 py-4 w-48">
                      <ProgressBar
                        ...
                        max={100}
                        color={
                          insp.alert_response_rate >= 90
                            ? "bg-green-400"
                            : insp.alert_response_rate >= 70
                              ? "bg-orange-400"
                              : "bg-red-400"
                        }
                      />
                    ...
                    <td className="px-4 py-4 w-48">
                      <ProgressBar
                        ...
                        color={
                          insp.production_registered === insp.production_total
                            ? "bg-green-400"
                            : insp.production_registered >= insp.production_total * 0.75
                              ? "bg-orange-400"
                              : "bg-red-400"
                        }
                      />
                    ...
                    <td className="px-4 py-4 text-center">
                      <span
                        className={`text-sm font-semibold ${insp.chance_loss_change.startsWith("-") ? "text-green-600" : "text-red-600"}`}
                      >
                    ...
                    <td className="px-4 py-4 text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        {cfg.icon}
                        <span
                          className={`rounded-full px-2.5 py-1 text-xs font-semibold ${cfg.className}`}
                        >
                          {cfg.label}
                        </span>
                      </div>
