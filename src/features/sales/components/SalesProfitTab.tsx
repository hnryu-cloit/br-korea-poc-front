    ...
    <div className="space-y-5">
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-7">
          {weeklyData.map((day) => (
            <div key={day.day} className="rounded-2xl bg-white px-4 py-4 shadow-sm">
              ...
              <p className="mt-1 text-sm font-bold text-green-600">순매출 {Math.round(day.net_revenue / 10000)}만</p>
              <p className="mt-1 text-xs text-slate-500">할인 {Math.round((day.revenue - day.net_revenue) / 1000)}천</p>
            </div>
          ))}
        </div>
      </article>
      <div className="grid gap-5 md:grid-cols-2">
        <article className="rounded-[24px] border border-border bg-white px-5 py-5 shadow-sm">
          ...
          <p className="mt-3 text-3xl font-bold text-green-600">{totalNetRevenue.toLocaleString()}원</p>
        ...
        <article className="rounded-[24px] border border-border bg-white px-5 py-5 shadow-sm">
          ...
          <p className="mt-3 text-3xl font-bold text-[#2454C8]">{totalRevenue.toLocaleString()}원</p>
